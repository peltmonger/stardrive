...
21. **Deployment** - for Cloudflare, adjust [wrangler.jsonc](../wrangler.jsonc). Disable **Speed Brain** in the Cloudflare dashboard (Speed → Optimization → Speed Brain) — Astro's own [prefetching](https://docs.astro.build/en/guides/prefetch/) conflicts with it and can cause double-fetches or stale content. If on-demand (SSR) collections are kept enabled in `theme.config.ts` (`onDemandRenderedCollections`), the `assets_navigation_has_no_effect` compatibility flag in `wrangler.jsonc` is **required** — without it, on-demand pages 404 in production (dev is unaffected). The flag is pre-set in the boilerplate; only remove it if all on-demand rendering is disabled.
