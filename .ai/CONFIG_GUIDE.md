...
20. **Cloudflare files** - only if on Cloudflare: adjust [`_headers`](../public/_headers) and [`_redirects`](../public/_redirects). Preserve the homepage `Link` header for agent discovery or point it to another useful machine-readable resource. For other hosts, configure an equivalent response header. For migrations, map old paths to the new structure.
21. **Deployment** - for Cloudflare, adjust [wrangler.jsonc](../wrangler.jsonc). Disable **Speed Brain** in the Cloudflare dashboard (Speed → Optimization → Speed Brain) — Astro's own [prefetching](https://docs.astro.build/en/guides/prefetch/) conflicts with it and can cause double-fetches or stale content.
